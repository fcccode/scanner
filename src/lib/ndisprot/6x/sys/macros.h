...
#if DBG
#define NPROT_REF_OPEN(_pOpen)   DbgRefOpen(_pOpen, __FILENUMBER, __LINE__)
#define NPROT_DEREF_OPEN(_pOpen) DbgDerefOpen(_pOpen, __FILENUMBER, __LINE__)
#else
#define NPROT_REF_OPEN(_pOpen)   RefOpen(_pOpen)
#define NPROT_DEREF_OPEN(_pOpen) DerefOpen(_pOpen)
#endif

//  Spinlock macros
#if DBG_SPIN_LOCK
#define NPROT_INIT_LOCK(_pLock)              AllocateSpinLock(_pLock, __FILENUMBER, __LINE__)
#define NPROT_ACQUIRE_LOCK(_pLock, DispatchLevel)           AcquireSpinLock(_pLock, DispatchLevel,__FILENUMBER, __LINE__)
#define NPROT_RELEASE_LOCK(_pLock,DispatchLevel)            ReleaseSpinLock(_pLock, DispatchLevel,__FILENUMBER, __LINE__)
#define NPROT_FREE_LOCK(_pLock)              FreeSpinLock(_pLock, __FILENUMBER, __LINE__)
#define NPROT_FREE_DBG_LOCK()                FreeDbgLock()
#else
...
#define NPROT_RCV_NBL_TO_LIST_ENTRY(_pNbl)   (&((PNPROT_RECV_NBL_RSVD)(NET_BUFFER_LIST_PROTOCOL_RESERVED(_pNbl)))->Link)
#define NPROT_RCV_NBL_FROM_LIST_ENTRY(_pEnt) (((PNPROT_RECV_NBL_RSVD)(CONTAINING_RECORD(_pEnt, NPROT_RECV_NBL_RSVD, Link)))->pNetBufferList)


// Send net buffer list context
#define NPROT_IRP_FROM_SEND_NBL(_pNbl)		(((PNPROT_SEND_NETBUFLIST_RSVD)((_pNbl)->Context->ContextData + (_pNbl)->Context->Offset))->pIrp)
#define NPROT_SEND_NBL_RSVD(_pNbl)          ((PNPROT_SEND_NETBUFLIST_RSVD)((_pNbl)->Context->ContextData + (_pNbl)->Context->Offset))


#define NPROT_REF_SEND_NBL(_pNbl)           (VOID)NdisInterlockedIncrement((PLONG)&NPROT_SEND_NBL_RSVD(_pNbl)->RefCount)
...
//  Memory allocation
#if DBG
#define NPROT_ALLOC_MEM(_pVar, _Size)        (_pVar) = AuditAllocMem((PVOID)&(_pVar), _Size, __FILENUMBER, __LINE__);
#define NPROT_FREE_MEM(_pMem)                AuditFreeMem(_pMem);
#else
#define NPROT_ALLOC_MEM(_pVar, _Size)        NdisAllocateMemoryWithTag((PVOID *)(&_pVar), (_Size), NPROT_ALLOC_TAG)
#define NPROT_FREE_MEM(_pMem)                NdisFreeMemory(_pMem, 0, 0)
#endif // DBG

#define NPROT_ZERO_MEM(_pMem, _ByteCount)           NdisZeroMemory(_pMem, _ByteCount)
#define NPROT_COPY_MEM(_pDst, _pSrc, _ByteCount)    NdisMoveMemory(_pDst, _pSrc, _ByteCount)
#define NPROT_MEM_CMP(_p1, _p2, _ByteCount)         NdisEqualMemory(_p1, _p2, _ByteCount)
#define NPROT_SET_MEM(_pMem, _ByteVal, _ByteCount)  NdisFillMemory(_pMem, _ByteCount, _ByteVal)
...
//  Flags
#define NPROT_SET_FLAGS(_FlagsVar, _Mask, _BitsToSet)    (_FlagsVar) = ((_FlagsVar) & ~(_Mask)) | (_BitsToSet)
#define NPROT_TEST_FLAGS(_FlagsVar, _Mask, _BitsToCheck)    (((_FlagsVar) & (_Mask)) == (_BitsToCheck))
