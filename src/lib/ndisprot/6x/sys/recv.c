...
--*/
{
    ...
    do {
        ...
        IoSetCancelRoutine(pIrp, CancelRead);
        ...
        ServiceReads(pOpenContext);//  Run the service routine for reads.
    } while (FALSE);
...
--*/
{
    ...
    pOpenContext = (POPEN_CONTEXT)pIrp->Tail.Overlay.DriverContext[0];
    NPROT_STRUCT_ASSERT(pOpenContext, oc);
...
--*/
{
    ...
    while (!NPROT_IS_LIST_EMPTY(&pOpenContext->PendedReads) && !NPROT_IS_LIST_EMPTY(&pOpenContext->RecvNetBufListQueue)) {
        ...
        while (pIrpEntry != &pOpenContext->PendedReads) {
            ...
            if (IoSetCancelRoutine(pIrp, NULL)) {//  It isn't being cancelled, and can't be cancelled henceforth.
                NPROT_REMOVE_ENTRY_LIST(pIrpEntry);
                FoundPendingIrp = TRUE;
                break;

                //  NOTE: we decrement PendedReadCount way below in the while loop, to avoid letting through a thread trying to unbind.
            } else {//  The IRP is being cancelled; let the cancel routine handle it.
        ...
        while (BytesRemaining && (pMdl != NULL) && SrcTotalLength) {
            ...
            if (pSrc == NULL) {
                DEBUGP(DL_FATAL, ("ServiceReads: Open %p, NdisQueryMdl failed for MDL %p\n", pOpenContext, pMdl));
                break;
            }
        ...
        FreeReceiveNetBufferList(pOpenContext, pRcvNetBufList, FALSE);
...
VOID ReceiveNetBufferLists(IN NDIS_HANDLE      ProtocolBindingContext,
...
/*++
Routine Description:
    Protocol entry point called by NDIS if the driver below uses NDIS 6 net buffer list indications.
    If the miniport allows us to hold on to this net buffer list, we use it as is, otherwise we make a copy.
Arguments:
...
--*/
{
    ...
    NPROT_STRUCT_ASSERT(pOpenContext, oc);
    if ((pOpenContext->State == Pausing) || (pOpenContext->State == Paused)) {
        if (NDIS_TEST_RECEIVE_CAN_PEND(ReceiveFlags) == TRUE) {
            NdisReturnNetBufferLists(pOpenContext->BindingHandle, pNetBufferLists, ReturnFlags);
        }
        return;
    }
    ...
    while (pNetBufList != NULL) {
        ...
        PMDL pMdl = NET_BUFFER_CURRENT_MDL(NET_BUFFER_LIST_FIRST_NB(pNetBufList));
        ...
        do {
            ASSERT(pMdl != NULL);
            if (pMdl) {
                NdisQueryMdl(pMdl, &pEthHeader, &BufferLength, NormalPagePriority | MdlMappingNoExecute);
            }
            ...
            BufferLength -= Offset;
            pEthHeader = (PETH_HEADER)((PUCHAR)pEthHeader + Offset);
            if (BufferLength < sizeof(ETH_HEADER)) {
                DEBUGP(DL_WARN, ("ReceiveNetBufferList: Open %p, runt nbl %p, first buffer length %d\n", pOpenContext, pNetBufList, BufferLength));
                break;
            }

            //  Check the EtherType.
            //  If the Ether type indicates presence of a tag, then the "real" Ether type is 4 bytes further down.
            if (pEthHeader->EthType == NPROT_8021P_TAG_TYPE) {
                USHORT UNALIGNED * pEthType;

                if (BufferLength < (sizeof(ETH_HEADER) + 4)) {
                    break;
                }
            ...
            //  If the miniport is out of resources, we can't queue this list of net buffer list - make a copy if this is so.
            ...
            if (NoReadIRP || NDIS_TEST_RECEIVE_CANNOT_PEND(ReceiveFlags)) {
                bAcceptedReceive = FALSE;
                pCopyNetBufList = AllocateReceiveNetBufferList(pOpenContext, TotalLength, &pCopyBuf);
                if (pCopyNetBufList == NULL) {
                    DEBUGP(DL_FATAL, ("ReceiveNetBufferList: Open %p, failed to alloc copy, %d bytes\n", pOpenContext, TotalLength));
                    break;
                }
                ...
                if (NtStatus != STATUS_SUCCESS) {
                    DEBUGP(DL_FATAL, ("ReceiveNetBufferList: Open %p, failed to copy the data, %d bytes\n", pOpenContext, TotalLength));
                    FreeReceiveNetBufferList(pOpenContext, pCopyNetBufList, DispatchLevel);// Free the NetBufferList and memory allocate before
                    break;
                }
            ...
            QueueReceiveNetBufferList(pOpenContext, pNetBufList, DispatchLevel);//  Queue this up and service any pending Read IRPs.
        } while (FALSE);

        // Ndisprot is not interested this NetBufferList, return the NetBufferList back to the miniport if the miniport gave us ownership of it
        if ((bAcceptedReceive == FALSE) && (NDIS_TEST_RECEIVE_CAN_PEND(ReceiveFlags) == TRUE)) {
    ...
    if (pReturnNetBufList != NULL) {
        NdisReturnNetBufferLists(pOpenContext->BindingHandle, pReturnNetBufList, ReturnFlags);
    }
}
...
Routine Description:
    ...
    If the queue size goes beyond a water mark, discard a Net Buffer list at the head of the queue.
...
--*/
{
    ...
    do {
        ...
        if ((pOpenContext->State == Paused) || (pOpenContext->State == Pausing)) {
            NPROT_RELEASE_LOCK(&pOpenContext->Lock, DispatchLevel);
            FreeReceiveNetBufferList(pOpenContext, pRcvNetBufList, DispatchLevel);
            break;
        }

        //  Check if the binding is in the proper state to receive this net buffer list.
        if (NPROT_TEST_FLAGS(pOpenContext->Flags, NPROTO_BIND_FLAGS, NPROTO_BIND_ACTIVE) && (pOpenContext->PowerState == NetDeviceStateD0)) {
            ...
            DEBUGP(DL_VERY_LOUD, ("QueueReceiveNetBufferList: open %p, queued nbl %p, queue size %d\n",
        ...
        if (pOpenContext->RecvNetBufListCount > MAX_RECV_QUEUE_SIZE) {
            ...
            FreeReceiveNetBufferList(pOpenContext, pDiscardNetBufList, DispatchLevel);

            NPROT_DEREF_OPEN(pOpenContext);  // dropped rcv packet - queue too long

            DEBUGP(DL_INFO, ("QueueReceiveNetBufferList: open %p queue too long, discarded  %p\n", pOpenContext, pDiscardNetBufList));
        } else {
            NPROT_RELEASE_LOCK(&pOpenContext->Lock, DispatchLevel);
        }

        ServiceReads(pOpenContext);//  Run the receive queue service routine now.
    } while (FALSE);
}


_Success_(return != 0)
PNET_BUFFER_LIST AllocateReceiveNetBufferList(_In_  POPEN_CONTEXT pOpenContext,
...
--*/
{
    ...
    *ppDataBuffer = NULL;
    ...
    do {
        ...
        if (pDataBuffer == NULL) {
            DEBUGP(DL_FATAL, ("AllocRcvNbl: open %p, failed to alloc data buffer %d bytes\n", pOpenContext, DataLength));
            break;
        }
        ...
        if (pMdl == NULL) {
            DEBUGP(DL_FATAL, ("AllocateRcvNbl: open %p, failed to alloc MDL, %d bytes\n", pOpenContext, DataLength));
            break;
        }

        pNetBufList = NdisAllocateNetBufferAndNetBufferList(pOpenContext->RecvNetBufferListPool,
                                                            0,          // ContextSize
                                                            0,          // ContextBackfill
                                                            pMdl,       // MdlChain
                                                            0,          // DataOffset
        ...
        if (pNetBufList == NULL) {
            DEBUGP(DL_FATAL, ("AllocateRcvNbl: open %p, failed to alloc Net Buffer List, %d bytes\n", pOpenContext, DataLength));
            break;
        }
...
Routine Description:
    ...
    is a local copy, free the net buffer list to our receive pool, else return this to the miniport.
Arguments:
    pOpenContext - pointer to open context
...
--*/
{
    ...
    do {
        if (NBL_TEST_PROT_RSVD_FLAG(pNetBufferList, NPROT_ALLOCATED_NBL)) {
            ...
            NdisQueryMdl(pMdl, (PVOID *)&pCopyData, &BufferLength, NormalPagePriority | MdlMappingNoExecute);
        ...
        NET_BUFFER_LIST_NEXT_NBL(pNetBufferList) = NULL;// The NetBufferList should be returned

        if (DispatchLevel) {
            NDIS_SET_RETURN_FLAG(ReturnFlags, NDIS_RETURN_FLAGS_DISPATCH_LEVEL);
        }

        NdisReturnNetBufferLists(pOpenContext->BindingHandle, pNetBufferList, ReturnFlags);
    } while (FALSE);
}
...
--*/
{
    ...
    while (!NPROT_IS_LIST_EMPTY(&pOpenContext->PendedReads)) {
        ...
        if (IoSetCancelRoutine(pIrp, NULL)) {
            ...
            //  we might end up examining the same (cancelled) IRP over and over again.
