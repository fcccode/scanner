...
--*/
{
    ...
    do {
        ...
        if (DataLength < sizeof(ETH_HEADER)) {
            DEBUGP(DL_WARN, ("Write: too small to be a valid packet (%d bytes)\n", DataLength));
            NtStatus = STATUS_BUFFER_TOO_SMALL;
            break;
        }

        if (DataLength > (pOpenContext->MaxFrameSize + sizeof(ETH_HEADER))) {
        ...
        if (pOpenContext->State != Running || pOpenContext->PowerState != NetDeviceStateD0) {
            NPROT_RELEASE_LOCK(&pOpenContext->Lock, FALSE);
        ...
        pNetBufferList = NdisAllocateNetBufferAndNetBufferList(pOpenContext->SendNetBufferListPool,
        ...
        //  Initialize the NetBufferList ref count.
        //  This NetBufferList will be freed when this count goes to zero.
        ...
        IoSetCancelRoutine(pIrp, CancelWrite);

        NPROT_RELEASE_LOCK(&pOpenContext->Lock, FALSE);

        NPROT_IRP_FROM_SEND_NBL(pNetBufferList) = pIrp;//  Set a back pointer from the packet to the IRP.
        ...
        NdisSendNetBufferLists(pOpenContext->BindingHandle, pNetBufferList, NDIS_DEFAULT_PORT_NUMBER, SendFlags);
    } while (FALSE);
...
--*/
{
    ...
    if (pOpenContext == NULL) {// The IRP is going to be completed from send complete function
    ...
    //  Try to locate the IRP in the pended write queue.
    //  The send completion routine may be running and might have removed it from there.
    NPROT_ACQUIRE_LOCK(&pOpenContext->Lock, FALSE);
    ...
    if (FoundIrp) {
        PVOID CancelId = pIrp->Tail.Overlay.DriverContext[2];
        //  Either the send completion routine hasn't run, or we got a peak
        //  at the IRP/netbufferlist before it had a chance to take it out of the pending IRP queue.
        //
        //  We do not complete the IRP here - note that we didn't dequeue it above.
        //  This is because we always want the send complete routine to complete the IRP.
        //  And this in turn is because the NetBufferlist that was prepared from the IRP has a buffer chain pointing to data associated with this IRP.
        //  Therefore we cannot complete the IRP before the driver below us is done with the data it pointed to.

        //  Request NDIS to cancel this send.
        //  The result of this call is that our SendComplete handler will be called (if not already called).
        DEBUGP(DL_INFO, ("CancelWrite: cancelling nbl %p on Open %p\n", pIrp->Tail.Overlay.DriverContext[1], pOpenContext));

        NdisCancelSendNetBufferLists(pOpenContext->BindingHandle, CancelId);
    }
...
VOID SendComplete(IN NDIS_HANDLE ProtocolBindingContext,
...
--*/
{
    ...
    for (CurrNetBufferList = pNetBufferList; CurrNetBufferList != NULL; CurrNetBufferList = NextNetBufferList) {
        ...
        NPROT_DEREF_SEND_NBL(CurrNetBufferList, DispatchLevel);//  We are done with the NDIS_PACKET:
        ...
        DEBUGP(DL_INFO, ("SendComplete: NetBufferList %p/IRP %p/Length %llu completed with status %x\n",
